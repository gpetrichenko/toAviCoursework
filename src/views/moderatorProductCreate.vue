<template>
  <div>
    ...
    <form class="container mb-5" id="form" @submit.stop.prevent="addToProductSubmit">
      ...
      <a href="#" class="btn btn-outline-success mx-3" @click="$router.go(-1)">Назад</a>
      <button type="submit" class="btn btn-outline-primary mx-3" :disabled="isProcessing">Новый товар</button>
    </form>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    async addToProductSubmit(e) {
      try {
        ...
        const { data, statusText } = await moderatorAPI.products.post({
          formData,
          images: this.images
        });
        ...
        Toast.fire({
          icon: "error",
          title: "Неудачное создание товара"
        });
      }
    },
    async fetchModeratorCategories() {
      try {
        ...
        Toast.fire({
          icon: "error",
          title: "Не удалось получит список категорий"
        });
      }
    }
  },
