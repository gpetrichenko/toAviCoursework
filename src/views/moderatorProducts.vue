    ...
    <div>
      ...
      <section id="main">
        <div class="container">
          <div class="row">
            <div class="col-md-3">
              <a class="nav-main-color list-group-item textac" style="color:white">
                Навигация
              </a>
              <a class="nav-item list-group-item list-group-item-action textac">
                <router-link :to="{name:'moderatorProducts'}" >Товары</router-link> </a>
                <p></p>
              </div>
              <div class="col-md-9">
...
export default {
  ...
  created() {
    ...
    this.fetchModeratorProducts({ page });
  },
  ...
  methods: {
    async deleteProduct(productId) {
      try {
        ...
        Toast.fire({
          icon: "error",
          title: "Продукт не может быть удален"
        });
      }
    },
    async fetchModeratorProducts({ page = 1 }) {
      try {
        ...
        Toast.fire({
          icon: "error",
          title: "Невозможно получить информацию о продуктах, попробуйте позже"
        });
      }
    }
  }
