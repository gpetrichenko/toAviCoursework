  ...
  methods: {
    async fetchAdminProduct(id) {
      try {
        ...
        Toast.fire({
          icon: "error",
          title: "Невозможно получить информацию о продукте, попробуйте позже"
        });
      }
    },
    async fetchAdminCategories() {
      try {
        this.$store.dispatch("updateLoading", true);
        const { data, statusText } = await moderatorAPI.categories.get();
        if (statusText !== "OK") {
        ...
        Toast.fire({
          icon: "error",
          title: "Невозможно получить информацию категории, попробуйте позже"
        });
      }
    },
    async handleSubmit(id) {
      try {
        ...
        const { data, statusText } = await moderatorAPI.products.put({
          id,
          formData
        });
        ...
        this.$router.push({ name: "moderatorProducts" });
      } catch (error) {
        this.$store.dispatch("updateProcessing", false);
        Toast.fire({
          icon: "error",
          title: "Изменить информацию не удалось, попробуйте позже"
        });
      }
    },
    async deleteImage(e) {
      try {
        ...
        const { data, statusText } = await moderatorAPI.products.deleteImage(
          imageid
        );
        if (statusText !== "OK" || data.status !== "success") {
          throw new Error(statusText);
        ...
        Toast.fire({
          icon: "error",
          title: "Удалить изображения не удалось, попробуйте немного позже"
        });
      }
    }
  }
...
.moderatorProductImg .prodcutImg {
  position: relative;
}
.prodcutImg .mask {
  display: flex;
